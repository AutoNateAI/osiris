...
function EntityDetailModal({ entity, onClose }: { entity: any; onClose: () => void }) {
  ...
  const rows = isPha
    ...
    : [['Branch', entity.branch], ['Role', entity.role], ['Chamber', entity.chamber || 'N/A'], ['Home Base', [entity.origin_city, entity.origin_state].filter(Boolean).join(', ') || 'Unknown'], ['Location Signal', entity.origin_kind === 'residence' ? 'Residence/home city' : entity.origin_kind === 'birthplace' ? 'Birthplace fallback' : 'Represented state fallback'], ['Represents', entity.represented_state || entity.state], ['Party', entity.party || 'Nonpartisan/Unknown'], ['District', entity.district ?? 'Statewide/N/A']];
  const bars = isPha
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[650] bg-black/60 backdrop-blur-sm flex items-center justify-center p-3">
      <motion.div initial={{ scale: 0.96, y: 12 }} animate={{ scale: 1, y: 0 }} className="glass-panel osiris-glow w-full max-w-3xl max-h-[86vh] overflow-y-auto styled-scrollbar">
        ...
        <div className="p-5 grid md:grid-cols-[1.1fr_0.9fr] gap-5">
          ...
          <div className="space-y-4">
            <div>
              <div className="hud-label mb-2">{isPha || isSbir ? 'FUNDING PROFILE' : 'POWER PROFILE'}</div>
              <div className="space-y-2">
                {bars.map(([label, value, color]) => (
                  <div key={label as string}>
                    <div className="flex justify-between text-[10px] mb-1"><span>{label}</span><span>{value}</span></div>
...
export default function Dashboard() {
  ...
  const [activeLayers, setActiveLayers] = useState({
    ...
    federal_power_judicial: true,
    federal_power_white_house: true,
    power_edges: true,
